...
Par ailleurs une clef API gratuite openweathermap.org est nécessaire (voir sur leur site). Créez un compte gratuit pour ensuite obtenir une clef API directement utilisable sur votre Companion. Les problèmes de réception des données OpenWeather ont été corrigés. La version 1 incluse zippée permet cependant d'utiliser le Companion sans les données météos, si vous ne souhaitez pas vous connecter sur OpenWeather. 
